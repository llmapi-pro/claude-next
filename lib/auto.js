...
async function runAuto(opts) {
  ...
  // If --resume <SLOT> was passed, the first window must boot from the
  // existing handoff via the `继续 <SLOT>` pass-phrase, not from PREAMBLE +
  // initialPrompt (which would ignore the handoff entirely). Without this
  // branch, --resume only took effect on the *second* window after a
  // mid-loop rotate — i.e. it was a no-op for its actual use case.
  let currentPrompt = resumeSlot
    ? `继续 ${resumeSlot}`
  ...
  try {
    while (!stopRequested) {
      ...
      if (!driver.exited) {
        ...
        // Historic note: 0.2.0-0.2.2 sent auto-confirm "继续" at +8s/+60s here
        // to unblock the /next skill's "3 秒内无异议" user-confirmation gate.
        // 0.2.3 fixed SKILL.md step 1 to never hard-pause, so those nudges
        // became stale. They were also harmful: extra stdin user messages
        // bumped driver.turns (now fixed in driver.js) and could distract
        // the child mid-handoff. The path now relies on waitForNewHandoff's
        // 300s timeout — if the audit subagent genuinely hangs we promote
        // the slot to verdict='aborted' and the loop continues without
        // injecting unnecessary user turns.
        try {
