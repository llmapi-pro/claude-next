...
class ClaudeDriver {
  ...
  send(userText) {
    ...
    // NB: do NOT increment _turnCount here. The rotator interprets `turns`
    // as assistant turn count (it caps via maxTurnsPerWindow to limit how
    // far one model-driven session can go). Counting user sends instead
    // mis-trips the cap whenever the orchestrator injects extra user
    // messages (e.g. /next + auto-confirm) on a single assistant turn.
    // _turnCount is bumped in _dispatch on `result` events, which fire
    // once per completed assistant turn.
  }
  ...
  _onStdout(buf) {
    ...
    // Sanity cap: a single NDJSON line should never realistically exceed
    // ~10 MB. If we cross that without seeing a newline, the child is
    // either misbehaving or hung — drop the buffer so we don't grow
    // memory unbounded, and surface an error so the orchestrator can react.
    if (this._buffer.length > 10 * 1024 * 1024) {
      const dropped = this._buffer.length;
      this._buffer = '';
      const err = new Error(`driver: dropped ${dropped} bytes of unterminated stdout (no newline within 10MB)`);
      for (const cb of this._listeners.error) cb(err);
    }
  }
  ...
  _dispatch(obj) {
    ...
    if (type === 'assistant' && obj.message && Array.isArray(obj.message.content)) {
      ...
      // Prefer the child's authoritative num_turns when reported, else
      // increment locally. The result event fires once per completed
      // assistant turn in stream-json mode.
      if (typeof obj.num_turns === 'number') this._turnCount = obj.num_turns;
      else this._turnCount += 1;
